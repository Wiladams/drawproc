...
static void
linear2_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
...
/* Read pixel at x, y*/
static DPPIXELVAL
linear2_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	register unsigned char *addr = psd->addr + y * psd->pitch + (x >> 2);
#if DEBUG
	assert(x >= 0 && x < psd->xres);
...
static void
linear2_drawhorzline(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y, DPPIXELVAL c)
{
...
static void
linear2_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	int	pitch = psd->pitch;
...
static void
linear2_blit(PSD dstpsd, DPCOORD dstx, DPCOORD dsty, DPCOORD w, DPCOORD h,
...
*/
static void
linear2_convblit_copy_mask_mono_byte_msb(PSD psd, PDPBLITPARMS gc)
{
	int		i;
