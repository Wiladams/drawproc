...
static int raster_ops[DPROP_MAX + 1] = {
	PIX_SRC,						/* DPROP_COPY*/
	PIX_SRC ^ PIX_DST,				/* DPROP_XOR*/
	PIX_SRC | PIX_DST,				/* DPROP_OR*/
	PIX_SRC & PIX_DST,				/* DPROP_AND*/
	PIX_CLR,						/* DPROP_CLEAR*/
	PIX_SET,						/* DPROP_SET*/
	PIX_NOT(PIX_SRC ^ PIX_DST),		/* DPROP_EQUIV*/
	PIX_NOT(PIX_SRC | PIX_DST),		/* DPROP_NOR*/
	PIX_NOT(PIX_SRC & PIX_DST),		/* DPROP_NAND*/
	PIX_NOT(PIX_DST),				/* DPROP_INVERT*/
	PIX_NOT(PIX_SRC),				/* DPROP_COPYINVERTED*/
	PIX_NOT(PIX_SRC) | PIX_DST,		/* DPROP_ORINVERTED*/
	PIX_NOT(PIX_SRC) & PIX_DST,		/* DPROP_ANDINVERTED*/
	PIX_SRC | PIX_NOT(PIX_DST),		/* DPROP_ORREVERSE*/
	PIX_SRC & PIX_NOT(PIX_DST),		/* DPROP_ANDREVERSE*/
	PIX_DST,						/* DPROP_NOOP*/
...
*/
void
GdRasterOp(PDPIMAGEHDR pixd, DPCOORD dx, DPCOORD dy, DPCOORD dw, DPCOORD dh, int op,
	PDPIMAGEHDR pixs, DPCOORD sx, DPCOORD sy)
{
	int dd;
	...
	l_int32    op)
{
	...
	l_uint32  *pdlwpart = 0;   /* ptr to last partial dest word */
	l_int32    i, j;
	...
	l_int32    sy)
{
	...
	l_int32    dfwbits;    /* first word dest bits in ovrhang */
	l_uint32  *pdfwpart=0;   /* ptr to first partial dest word */
	l_uint32  *psfwpart=0;   /* ptr to first partial src word */
	l_int32    dfwfullb;   /* boolean (1, 0) if there exists a full dest word */
	l_int32    dnfullw;    /* number of full words in dest */
	l_uint32  *pdfwfull = 0;   /* ptr to first full dest word */
	l_uint32  *psfwfull=0;   /* ptr to first full src word */
	l_int32    dlwpartb;   /* boolean (1, 0) if last dest word is partial */
	l_uint32   dlwmask;    /* mask for last partial dest word */
	l_int32    dlwbits;    /* last word dest bits in ovrhang */
	l_uint32  *pdlwpart = 0;   /* ptr to last partial dest word */
	l_uint32  *pslwpart=0;   /* ptr to last partial src word */
	l_int32    i, j;
