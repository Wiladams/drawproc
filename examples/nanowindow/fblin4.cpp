...
static void
linear4_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
...
/* Read pixel at x, y*/
static DPPIXELVAL
linear4_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	register unsigned char *addr = psd->addr + y * psd->pitch + (x >> 1);
	DPPIXELVAL	c;
#if DEBUG
...
static void
linear4_drawhorzline(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y, DPPIXELVAL c)
{
...
static void
linear4_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	int	pitch = psd->pitch;
...
/* srccopy bitblt*/
static void
linear4_blit(PSD dstpsd, DPCOORD dstx, DPCOORD dsty, DPCOORD w, DPCOORD h,
...
*/
static void
linear4_convblit_copy_mask_mono_byte_msb(PSD psd, PDPBLITPARMS gc)
{
	/*
