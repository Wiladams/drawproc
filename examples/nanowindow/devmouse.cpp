...
static DPCOORD	xpos;		/* current x position of mouse */
static DPCOORD	ypos;		/* current y position of mouse */
static DPCOORD	minx;		/* minimum allowed x position */
static DPCOORD	maxx;		/* maximum allowed x position */
static DPCOORD	miny;		/* minimum allowed y position */
static DPCOORD	maxy;		/* maximum allowed y position */
...
static DPCOORD 	curminx;	/* minimum x value of cursor */
static DPCOORD 	curminy;	/* minimum y value of cursor */
static DPCOORD 	curmaxx;	/* maximum x value of cursor */
static DPCOORD 	curmaxy;	/* maximum y value of cursor */
static int	curvisible;	/* >0 if cursor is visible*/
static bool 	curneedsrestore;/* cursor needs restoration after drawing*/
static DPCOORD 	cursavx;	/* saved cursor location*/
static DPCOORD 	cursavy;
static DPCOORD	cursavx2;
static DPCOORD	cursavy2;
static DPPIXELVALHW curfg;		/* foreground color of cursor */
static DPPIXELVALHW curbg;		/* background color of cursor */
static DPPIXELVALHW cursavbits[DPMAX_CURSOR_SIZE * DPMAX_CURSOR_SIZE];
static DPIMAGEBITS cursormask[DPMAX_CURSOR_BUFLEN];
static DPIMAGEBITS cursorcolor[DPMAX_CURSOR_BUFLEN];
...
GdOpenMouse(void)
{
	...
	curminx = minx;
	curminy = miny;
	curmaxx = curminx + DPMAX_CURSOR_SIZE - 1;
	curmaxy = curminy + DPMAX_CURSOR_SIZE - 1;
...
*/
void
GdRestrictMouse(DPCOORD newminx, DPCOORD newminy, DPCOORD newmaxx,
	DPCOORD newmaxy)
{
	minx = newminx;
	miny = newminy;
...
*/
void
GdMoveMouse(DPCOORD newx, DPCOORD newy)
{
...
*/
int
GdReadMouse(DPCOORD *px, DPCOORD *py, int *pb)
{
	DPCOORD	x, y, z;
...
*/
void
GdMoveCursor(DPCOORD newx, DPCOORD newy)
{
	DPCOORD shiftx;
	DPCOORD shifty;
...
*/
bool
GdGetCursorPos(DPCOORD *px, DPCOORD *py)
{
	*px = xpos;
	*py = ypos;
...
GdSetCursor(PDPCURSOR pcursor)
{
	...
	bytes = DPIMAGE_SIZE(pcursor->width, pcursor->height) * sizeof(DPIMAGEBITS);
...
GdShowCursor(PSD psd)
{
	DPCOORD 		x;
	DPCOORD 		y;
	DPPIXELVALHW *	saveptr;
	DPIMAGEBITS *	cursorptr;
	DPIMAGEBITS *	maskptr;
	DPIMAGEBITS 	curbit, cbits = 0, mbits = 0;
	DPPIXELVALHW 	oldcolor;
	DPPIXELVALHW 	newcolor;
	int 		oldmode;
	...
	*/
	...
	for (y = curminy; y <= curmaxy; y++) {
		if (curbit != DPIMAGE_FIRSTBIT) {
			cbits = *cursorptr++;
			mbits = *maskptr++;
			curbit = DPIMAGE_FIRSTBIT;
		}
		for (x = curminx; x <= curmaxx; x++) {
			...
			curbit = DPIMAGE_NEXTBIT(curbit);
			if (!curbit) {	/* check > one DPIMAGEBITS wide*/
				cbits = *cursorptr++;
				mbits = *maskptr++;
				curbit = DPIMAGE_FIRSTBIT;
			}
...
*/
void
GdCheckCursor(PSD psd, DPCOORD x1, DPCOORD y1, DPCOORD x2, DPCOORD y2)
{
	DPCOORD temp;
...
static DPTRANSFORM g_trans;	/* current transform*/
...
void
GdSetTransform(DPTRANSFORM *trans)
{
	if (!(mousedev.flags & MOUSE_TRANSFORM))
		return;
