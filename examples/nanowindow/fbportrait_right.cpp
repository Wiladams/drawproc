...
void
fbportrait_right_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
	psd->orgsubdriver->DrawPixel(psd, psd->yvirtres - y - 1, x, c);
}

DPPIXELVAL
fbportrait_right_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	return psd->orgsubdriver->ReadPixel(psd, psd->yvirtres - y - 1, x);
}

void
fbportrait_right_drawhorzline(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y, DPPIXELVAL c)
{
	/*x2 = x2;
	while (x2 <= x1)
...
void
fbportrait_right_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	/*while (y1 <= y2)
...
void
fbportrait_right_fillrect(PSD psd, DPCOORD x1, DPCOORD y1, DPCOORD x2, DPCOORD y2,
	DPPIXELVAL c)
{
...
void
fbportrait_right_blit(PSD dstpsd, DPCOORD destx, DPCOORD desty, DPCOORD w, DPCOORD h,
...
void
fbportrait_right_convblit_blend_mask_alpha_byte(PSD dstpsd, PDPBLITPARMS gc)
{
...
void
fbportrait_right_convblit_copy_mask_mono_byte_msb(PSD psd, PDPBLITPARMS gc)
{
...
void
fbportrait_right_convblit_copy_mask_mono_byte_lsb(PSD psd, PDPBLITPARMS gc)
{
	ADDR8 pixel_in, pixel_out;
	DPCOORD	in_x, in_y, in_w, in_h;
	DPCOORD	out_x, out_y, out_w, out_h;
