...
static void
linear16_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
...
/* Read pixel at x, y*/
static DPPIXELVAL
linear16_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	register unsigned char *addr = psd->addr + y * psd->pitch + (x << 1);
#if DEBUG
	assert(x >= 0 && x < psd->xres);
...
static void
linear16_drawhorzline(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y, DPPIXELVAL c)
{
...
static void
linear16_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	int	pitch = psd->pitch;
