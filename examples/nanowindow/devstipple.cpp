...
extern DPSTIPPLE gr_stipple;		/* The current stipple as set by the GC */
extern DPTILE gr_tile;			/* The current tile as set by the GC */
extern DPPOINT gr_ts_offset;		/* The x and y offset of the tile / stipple */

static int ts_origin_x = 0;
static int ts_origin_y = 0;

/* Some useful macros */
#define SPITCH ((gr_stipple.width + (DPIMAGE_BITSPERIMAGE - 1)) / DPIMAGE_BITSPERIMAGE)

/* This is when the bits are least significantly aligned */
/* #define BIT_SET(data, w, h) \
(data[(h * SPITCH) + (w / DPIMAGE_BITSPERIMAGE)] & (1 << (w % DPIMAGE_BITSPERIMAGE))) */

/* This is when the bits are most significantly aligned */
#define BIT_SET(data, w, h) \
	(data[(h * SPITCH) + (w / DPIMAGE_BITSPERIMAGE)] & (1 << ((DPIMAGE_BITSPERIMAGE - 1) - (w % DPIMAGE_BITSPERIMAGE))))

void
GdSetStippleBitmap(DPIMAGEBITS *stipple, DPCOORD width, DPCOORD height)
{
	int size;
	...
	size = DPIMAGE_SIZE(width, height) * sizeof(DPIMAGEBITS);
	gr_stipple.bitmap = (DPIMAGEBITS *)malloc(size);
	if (!gr_stipple.bitmap)
		return;
...
void
GdSetTilePixmap(PSD src, DPCOORD width, DPCOORD height)
{
	gr_tile.psd = src;
	if (!src) {
		gr_tile.width = 0;
...
static void
tile_drawrect(PSD psd, DPCOORD x, DPCOORD y, DPCOORD w, DPCOORD h)
{
	int sx = x, sy = y;
	int px = 0, py = 0;
	int dw = w;
	...
	while (dh) {
		...
		while (dw) {
			...
			GdBlit(psd, sx + px, sy + py, cw, ch, gr_tile.psd,
				(tilex + px) % gr_tile.width, (tiley + py) % gr_tile.height, DPROP_COPY);
			dw -= cw;
			px += cw;
		}
		dh -= ch;
...
void
ts_drawpoint(PSD psd, DPCOORD x, DPCOORD y)
{
	...
	if (gr_fillmode == DPFILL_STIPPLE || gr_fillmode == DPFILL_OPAQUE_STIPPLE) 
	{
...
void
ts_drawrow(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y)
{
	int x;
...
void
ts_fillrect(PSD psd, DPCOORD x, DPCOORD y, DPCOORD w, DPCOORD h)
{
	int x1 = x;
	int x2 = x + w - 1;
	int y1 = y;
