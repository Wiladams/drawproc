...
#else
#define FILEP  FILE *
#define FOPEN(pfile, path, mode)           fopen_s(&pfile, path, mode)
#define FREAD(file, buffer, size)   fread(buffer, 1, size, file)
#define FSEEK(file, offset, whence) fseek(file, offset, whence)
#define FCLOSE(file)                fclose(file)
#endif

/* Handling routines for FNT fonts, use DPCOREFONT structure */
PDPFONT fnt_createfont(const char *name, DPCOORD height, DPCOORD width, int attr);
static void fnt_unloadfont(PDPFONT font);
static PDPCFONT fnt_load_font(const char *path);

/* these procs used when font ASCII indexed*/
DPFONTPROCS fnt_fontprocs = {
	0,				/* can't scale*/
...
static DPFONTPROCS fnt_fontprocs16 = {
	0,				/* can't scale*/
	DPTF_UC16,		/* large font, expect UC16*/
	NULL,			/* init*/
	fnt_createfont,
	gen_getfontinfo,
	gen_gettextsize,
...
/* load font and allocate DPCOREFONT structure*/
PDPFONT
fnt_createfont(const char *name, DPCOORD height, DPCOORD width, int attr)
{
	PDPCOREFONT	pf;
	PDPCFONT	cfont;
	int		uc16;
	...
	if (!(pf = (DPCOREFONT *)malloc(sizeof(DPCOREFONT)))) {
		free(cfont);
		return NULL;
	}
...
void
fnt_unloadfont(PDPFONT font)
{
	PDPCOREFONT pf = (PDPCOREFONT)font;
	PDPCFONT    pfc = pf->cfont;
...
fnt_load_font(const char *path)
{
	...
	ifperr = FOPEN(ifp, path, "rb");
	if (ifperr != 0) {
		sprintf_s(fname, "%s/%s", FNT_FONT_DIR, path);
		ifperr = FOPEN(ifp, fname, "rb");

		/* Try to grab it from the MWFONTDIR directory */
		if (ifp!= 0) {
			size_t envSize = 0;
			char *envBuff;
			errno_t envErr = _dupenv_s(&envBuff, &envSize, "DPFONTDIR");
			if (envErr == 0) {
				sprintf_s(fname, "%s/%s", envBuff, path);
	...
	pf = (PDPCFONT)calloc(1, sizeof(DPCFONT));
	if (!pf)
		goto errout;
	...
	if (!pf->name)
		goto errout;
	strcpy_s(pf->name, DPLF_FACESIZE, name);
	...
	if (!READLONG(ifp, &nbits))
		goto errout;
	pf->bits = (DPIMAGEBITS *)malloc(nbits * sizeof(DPIMAGEBITS));
	if (!pf->bits)
		goto errout;
	pf->bits_size = nbits;
