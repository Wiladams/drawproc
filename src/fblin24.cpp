...
static void
linear24_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
...
/* Read pixel at x, y*/
static DPPIXELVAL
linear24_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	register unsigned char *addr = psd->addr + y * psd->pitch + x * 3;
#if DEBUG
	assert(x >= 0 && x < psd->xres);
...
static void
linear24_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	int	pitch = psd->pitch;
