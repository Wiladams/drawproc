...
#include "dpdevice.h"
#include "genmem.h"
#include "genfont.h"		// NUMBER_FONTS
#include "fb.h"
...
select_fb_subdriver(PSD psd)
{
	...
	if (psd->planes == 1) {
		...
		case 24:
			...
			break;
		case 32:
			if (psd->data_format == DPIF_RGBA8888)	/* RGBA pixmaps*/
...
set_data_format(PSD psd)
{
	...
	case DPPF_TRUECOLOR332:
		data_format = DPIF_RGB332;
		break;
	case DPPF_TRUECOLOR233:
		data_format = DPIF_BGR233;
		break;
	case DPPF_PALETTE:
		switch (psd->bpp) {
		case 8:
			data_format = DPIF_PAL8;
			break;
		case 4:
			data_format = DPIF_PAL4;
			break;
		case 2:
			data_format = DPIF_PAL2;
			break;
		case 1:
			data_format = DPIF_PAL1;
			break;
		}
		break;
	}
...
void
gen_getscreeninfo(PSD psd, PDPSCREENINFO psi)
{
