...
static void
linear8_drawpixel(PSD psd, DPCOORD x, DPCOORD y, DPPIXELVAL c)
{
...
/* Read pixel at x, y*/
static DPPIXELVAL
linear8_readpixel(PSD psd, DPCOORD x, DPCOORD y)
{
	register unsigned char *addr = psd->addr + y * psd->pitch + x;
#if DEBUG
	assert(x >= 0 && x < psd->xres);
...
static void
linear8_drawhorzline(PSD psd, DPCOORD x1, DPCOORD x2, DPCOORD y, DPPIXELVAL c)
{
...
static void
linear8_drawvertline(PSD psd, DPCOORD x, DPCOORD y1, DPCOORD y2, DPPIXELVAL c)
{
	int	pitch = psd->pitch;
	...
	for (r = 0; r<32; ++r) {
		for (g = 0; g<32; ++g)
			for (b = 0; b<32; ++b)
				...
				GdFindNearestColor(gr_palette, 256, DPRGB(r << 3, g << 3, b << 3));
	}
	return 1;
}
...
*/
static void
linear8_convblit_copy_mask_mono_byte_lsb(PSD psd, PDPBLITPARMS gc)
{
	/*
...
*/
static void
linear8_convblit_copy_mask_mono_byte_msb(PSD psd, PDPBLITPARMS gc)
{
	/*
	* The difference between the MSB_FIRST and LSB_FIRST variants of
	* this function is simply the definition of these three #defines.
...
*/
static void
linear8_convblit_blend_mask_alpha_byte(PSD psd, PDPBLITPARMS gc)
{
	ADDR8 dst, alpha;
	int x, y;
	unsigned int as;
