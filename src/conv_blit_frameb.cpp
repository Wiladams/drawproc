...
\
	case LEFT: \
		...
		gc->dstx = tmp; \
\
		dsz = -((int)gc->dst_pitch);		/* dst: next row up*/ \
...
\
		...
		dst_pitch = -((int)gc->dst_pitch);	/* dst: next pixel up*/ \
		break; \
	} \
}
...
static inline void ALWAYS_INLINE frameblit_blit(PSD psd, PDPBLITPARMS gc,
	...
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	...
	if (op == DPROP_SRC_OVER && psd->bpp != 32)
		op = DPROP_COPY;
...
void frameblit_xxxa8888(PSD psd, PDPBLITPARMS gc)
{
...
static inline void ALWAYS_INLINE frameblit_stretchblit(PSD psd, PDPBLITPARMS gc,
	int SSZ, int SR, int SG, int SB, int SA,
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
...
void
frameblit_stretch_xxxa8888(PSD psd, PDPBLITPARMS gc)
{
...
void
frameblit_stretch_24bpp(PSD psd, PDPBLITPARMS gc)
{
...
void
frameblit_stretch_16bpp(PSD psd, PDPBLITPARMS gc)
{
...
void
frameblit_stretch_8bpp(PSD psd, PDPBLITPARMS gc)
{
...
void
frameblit_stretch_rgba8888_bgra8888(PSD psd, PDPBLITPARMS gc)
{
...
void
frameblit_stretch_rgba8888_bgr888(PSD psd, PDPBLITPARMS gc)
{
	/* works for src_over and copy only*/
...
void
frameblit_stretch_rgba8888_16bpp(PSD psd, PDPBLITPARMS gc)
{
	/* works for src_over and copy only*/
