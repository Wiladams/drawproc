...
static PSD GdDecodeImage(buffer_t &src, const char *path, int flags);
...
*/
void
GdDrawImageFromBuffer(PSD psd, DPCOORD x, DPCOORD y, DPCOORD width,
...
*/
void
GdDrawImageFromFile(PSD psd, DPCOORD x, DPCOORD y, DPCOORD width, DPCOORD height,
...
*/
PSD
GdLoadImageFromFile(const char *path, int flags)
{
	int fd;
	PSD	pmd;
	void *buffer = 0;
...
{
	DPCOORD minx = gc->srcx;
	DPCOORD maxx = minx + gc->width;
...
GdConvertImageRGBA(PSD pmd)
{
	...
	/* create RGBA pixmap*/
	rgba = GdCreatePixmap(&scrdev, pmd->xvirtres, pmd->yvirtres, DPIF_RGBA8888, NULL, 0);
	if (!rgba)
		return pmd;
...
*/
static PSD
GdDecodeImage(buffer_t &src, const char *path, int flags)
{
	PSD	pmd = NULL;
	int	op;
	...
	op = (pmd->data_format & DPIF_HASALPHA) ? DPROP_SRC_OVER : DPROP_COPY;
	if (scrdev.pixtype != DPPF_PALETTE && !GdFindConvBlit(pmd, pmd->data_format, op))
...
*/
void
GdDrawImagePartToFit(PSD psd, DPCOORD x, DPCOORD y, DPCOORD width, DPCOORD height,
	DPCOORD sx, DPCOORD sy, DPCOORD swidth, DPCOORD sheight, PSD pmd)
{
	PSD			pmd2;
	DPCLIPRECT	rcDst, rcSrc;
	...
	if (height == pmd->yvirtres && width == pmd->xvirtres && swidth == 0) {
		GdDrawImage(psd, x, y, (PDPIMAGEHDR)pmd);	// FIXME casting MWIMAGEHDR
		return;
	}
	...
	GdFreePixmap(pmd2);
#else
	GdStretchBlit(psd, x, y, width, height, pmd, sx, sy, sx + swidth, sy + sheight, DPROP_COPY);
#endif
}
...
*/
bool
GdGetImageInfo(PSD pmd, PDPIMAGEINFO pinfo)
{
	int		i;
