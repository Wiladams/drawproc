...
static void
big5_gettextbits(PDPFONT pfont, int ch, const DPIMAGEBITS **retmap,
...
static void
euccn_gettextbits(PDPFONT pfont, int ch, const DPIMAGEBITS **retmap,
	DPCOORD *pwidth, DPCOORD *pheight, DPCOORD *pbase)
{
	unsigned int	CH = ch >> 8;
	unsigned int	CL = ch & 0xFF;
...
static void
jis_gettextbits(PDPFONT pfont, int ch, const DPIMAGEBITS **retmap,
	DPCOORD *pwidth, DPCOORD *pheight, DPCOORD *pbase)
{
	unsigned int	CH = ch >> 8;
	unsigned int	CL = ch & 0xFF;
	...
	*pheight = 12;
	*pbase = 0;
	*retmap = (DPIMAGEBITS *)map;
}
#endif /*HAVE_JISX0213_SUPPORT*/

#if HAVE_KSC5601_SUPPORT
static void
euckr_gettextbits(PDPFONT pfont, int ch, const DPIMAGEBITS **retmap,
	DPCOORD *pwidth, DPCOORD *pheight, DPCOORD *pbase)
{
...
void
dbcs_gettextbits(PDPFONT pfont, int ch, DPTEXTFLAGS flags,
	const DPIMAGEBITS **retmap, DPCOORD *pwidth, DPCOORD *pheight,
	DPCOORD *pbase)
{
	/* gettextbits function*/
	void(*func)(PDPFONT pfont, int ch, const DPIMAGEBITS **retmap,
		DPCOORD *pwidth, DPCOORD *pheight, DPCOORD *pbase);

	if (ch >= 0x0100) {	/* character was DBCS-encoded*/
		switch (flags & DPTF_DBCSMASK) {
#if HAVE_BIG5_SUPPORT
		case DPTF_DBCS_BIG5:
...
#endif
#if HAVE_GB2312_SUPPORT
		case DPTF_DBCS_EUCCN:
			func = euccn_gettextbits;
			break;
#endif
#if HAVE_KSC5601_SUPPORT
		case DPTF_DBCS_EUCKR:
...
*/
void
dbcs_gettextsize(PDPFONT pfont, const unsigned short *str, int cc,
	DPTEXTFLAGS flags, DPCOORD *pwidth, DPCOORD *pheight, DPCOORD *pbase)
{
