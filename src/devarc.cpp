...
* DPPIE is a filled shape, like a section of a pie chart.
*/
void
GdArcAngle(PSD psd, DPCOORD x0, DPCOORD y0, DPCOORD rx, DPCOORD ry,
...
typedef struct {
	PSD	psd;
	DPCOORD	x0, y0;		/* center*/
	DPCOORD	rx, ry;		/* radii*/
	DPCOORD	ax, ay;		/* start point*/
	DPCOORD	bx, by;		/* end point*/
	int	adir;		/* start pt: 1=bottom half, -1=top half*/
	int	bdir;		/* end pt:  -1=bottom half,  1=top half*/
	int	type;		/* MWARC, MWARCOUTLINE, DPPIE, DPELLIPSE etc*/
} SLICE;
...
*/
static int
clip_line(SLICE *slice, DPCOORD xe, DPCOORD ye, int dir, DPCOORD y, DPCOORD *x0,
	DPCOORD *x1)
{
#if 1
	/*
...
static void
draw_line(SLICE *slice, DPCOORD x0, DPCOORD y, DPCOORD x1, int mode)
{
...
*/
static void
drawarcsegment(SLICE *slice, DPCOORD xp, DPCOORD yp, int drawon)
{
	uint32_t dm = 0;
	int dc = 0;
	...
	case DPPIE:
		...
		if (gr_fillmode == DPFILL_SOLID) GdSetDash(&dm, &dc);
...
drawarc(SLICE *slice)
{
	...
	DPCOORD xp, yp;		/* current point (based on center) */
	DPCOORD rx, ry;
	...
	while (dx < dy) {
		...
		*/
		if ((d > 0) || ((slice->type != DPPIE) && (slice->type != DPELLIPSEFILL))) {
			if (gr_dashcount) {
...
*/
void
GdArc(PSD psd, DPCOORD x0, DPCOORD y0, DPCOORD rx, DPCOORD ry,
...
*/
void
GdEllipse(PSD psd, DPCOORD x, DPCOORD y, DPCOORD rx, DPCOORD ry, bool fill)
{
	SLICE	slice;
	...
	*/
	...
	slice.rx = rx;
	slice.ry = ry;
	slice.type = fill ? DPELLIPSEFILL : DPELLIPSE;
